...
{
	if (m.coef == 0) throw invalid_argument("Ќевозможно добавить моном с нулевым коэффициентом");
	this->Reset();
	bool isAdded = false;
	while (!this->IsEnd()) {
...
{
	if (other.IsEmpty()) throw invalid_argument("Ќе удаетс€ добавить пустой многочлен");
	TNode<TMonom>* current = other.pHead->pNext;
	while (current != nullptr) {
		this->AddMonom(current->value);
		current = current->pNext;
...
{
	if (this->IsEmpty()) throw invalid_argument("Ќевозможно умножить пустой многочлен");
